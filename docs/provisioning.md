...
The command is idempotent only when the same tenant slug is already owned by the selected owner and has a salon. Any conflicting slug fails without changing PostgreSQL.
